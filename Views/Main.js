  ...
  const panResponder = useRef(
    PanResponder.create({
      ...
      onPanResponderMove: (event, gesture) => {
        Animated.spring(
            pan, { x:gesture.moveX, y: gesture.moveY, useNativeDriver: true,}
        );
        Animated.event(
            [
                null,
      ...
      onPanResponderRelease: (event, gesture) => {
        ...
        console.log(gesture.moveX + " " + gesture.moveY + gesture.dx + " " + gesture.dy);
        
        // if (gesture.moveX <= Dimensions.get('window').width *3/4) {
        if (gesture.moveX <= windowWidth * 3/4) {
          Animated.spring(pan, {
            toValue: { x:0, y:0 },
            friction: 1,
            tension: 100,
